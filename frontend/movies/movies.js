document.addEventListener('DOMContentLoaded', async () => {
    // 1. Get the TMDB Key using your cached function!
    let API_KEY = "";
    try {
        API_KEY = await getTMDBKey();
    } catch (error) {
        console.error("Failed to load TMDB key:", error);
        return; // Stop execution if we can't get the key
    }
        ...
        try {
            ...
            if (currentType === 'both') {
                ...
                const [movieRes, tvRes] = await Promise.all([
                    // ✅ FIXED: Changed TMDB_API_KEY to API_KEY
                    fetch(`${TMDB_BASE}/discover/movie?api_key=${API_KEY}&language=en-US&page=${currentPage}&sort_by=${currentSort}${currentGenre ? `&with_genres=${currentGenre}` : ''}`),
                    fetch(`${TMDB_BASE}/discover/tv?api_key=${API_KEY}&language=en-US&page=${currentPage}&sort_by=${currentSort}${currentGenre ? `&with_genres=${currentGenre}` : ''}`)
                ]);
                ...
                // ✅ FIXED: Changed TMDB_API_KEY to API_KEY
                const res = await fetch(`${TMDB_BASE}/discover/${endpoint}?api_key=${API_KEY}&language=en-US&page=${currentPage}&sort_by=${currentSort}${currentGenre ? `&with_genres=${currentGenre}` : ''}`);
            ...
            if (currentPage > totalPages && endMessage) endMessage.style.display = 'block';
        }
    }
